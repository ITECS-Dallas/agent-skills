...
PLATFORM=$(uname -s)
case "$PLATFORM" in
  Darwin*) OP_COMMAND="/opt/homebrew/bin/op-itecs" ;;
  Linux*) OP_COMMAND=$(command -v op-itecs || true) ;;
  *) printf 'HaloPSA setup supports macOS and Linux.\n' >&2; exit 2 ;;
esac
...
usage() {
  cat <<'EOF'
Usage: configure-halopsa-mcp.sh --technician "Full Name" [options]

Options:
  --technician NAME  Exact HaloPSA technician name (required)
  --op-command PATH  Prompt-free ITECS 1Password wrapper
                     (macOS: /opt/homebrew/bin/op-itecs; Linux: op-itecs on PATH)
...
[[ -n "$OP_COMMAND" ]] || fail "op-itecs was not found on PATH; supply --op-command /absolute/path/to/op-itecs"
...
printf 'Command-backed config written to %s. Start a new Codex CLI session or restart Codex Desktop and start a new task.\n' "$CONFIG_PATH"
