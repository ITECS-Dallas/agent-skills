...
PLATFORM="${1:-Darwin}"
case "$PLATFORM" in
  Darwin) ENTRYPOINT=macos ;;
  Linux) ENTRYPOINT=linux ;;
  *) printf 'Unsupported test platform.\n' >&2; exit 2 ;;
esac
CONFIGURATOR="$ROOT/scripts/configure-halopsa-mcp-$ENTRYPOINT.sh"
...
const fs = require('fs');
if ((fs.statSync(process.argv[2]).mode & 0o777) !== 0o600) throw new Error('config mode is not 600');
EOF
...
# Linux should use the existing prompt-free wrapper on PATH without a new secret store.
if [[ "$PLATFORM" == Linux ]]; then
  DEFAULT_CONFIG="$TEST_ROOT/default-linux.json"
  PATH="$FAKE_BIN:$PATH" "$CONFIGURATOR" --technician "Daniel Moran" --config "$DEFAULT_CONFIG" >/dev/null
  node - "$DEFAULT_CONFIG" "$FAKE_BIN/op-itecs" <<'EOF'
const fs = require('fs');
const server = JSON.parse(fs.readFileSync(process.argv[2], 'utf8')).servers[0];
if (server.client_secret_command[0] !== process.argv[3]) throw new Error('Linux did not resolve op-itecs from PATH');
EOF
fi

printf '%s HaloPSA configurator tests passed\n' "$PLATFORM"
