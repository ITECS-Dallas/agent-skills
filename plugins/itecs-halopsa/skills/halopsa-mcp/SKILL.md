...
- For a project create, field update, or status update, use the equivalent `halopsa.projects.*` tool. Follow its exact approval phrase, use `last_update` for field patches, and revalidate current/allowed statuses for status changes.
...
- `halopsa.projects.create` - create one approved project with typed routing, dates, categories, milestone, and budget fields.
